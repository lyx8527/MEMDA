...
def calculate(alpha, Times):
    ...
    for times in range(Times):
        ...
        for cv in range(5):
            ...
            A3_1 = utils.WKNKN(torch.tensor(A2, device=device).cpu().numpy(), D_Similarity[1].cpu().numpy(),
                             M_Similarity[1].cpu().numpy(), 20, 0.9)
